...
export default function OfferLifecyclePage() {
  ...
  useEffect(() => {
    ...
    if (savedId) {
      ...
      setOfferId(id);
      api.getOfferById(id).then((offer: any) => {
        if (offer?.money_offered) {
          setFormData(prev => ({ ...prev, totalOfferValue: offer.money_offered }));
        }
      }).catch(console.error);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    // Use PCA results if available
    if (pcaResult?.summary) {
      setFormData(prev => ({
        ...prev,
        totalOfferValue: pcaResult.summary.totalInvestment || prev.totalOfferValue,
        yo: pcaResult.summary.totalNetRevenue || calculatedYo,
        gm: pcaResult.summary.gmPool || calculatedGm,
        roc: pcaResult.financials?.irr || calculatedRoc,
        finalDofa: pcaResult.dofa?.level || prev.finalDofa,
  ...
  const handleValidation = async () => {
    ...
    try {
      const result = await api.validateOffer(offerId);
      setValidationResults(result);
    } catch (err) {
      console.error('Validation failed:', err);
      setValidationResults({ isValid: false, checks: [{ name: 'System Error', status: 'fail', message: 'Validation service unavailable' }] });
    } finally {
  ...
  const handleGenerateLetter = async () => {
    ...
    try {
      const url = api.getOfferLetterUrl(offerId);
  ...
  const handleProceed = async () => {
    ...
    try {
      await api.submitOfferForApproval({ offerId });
      localStorage.removeItem('odt_current_offer_id');
