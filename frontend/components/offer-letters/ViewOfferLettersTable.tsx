...
import { offerLetters, offerLetterTypes, OfferLetterStatus } from "@/lib/offerLetters";
import { inputClass } from "@/components/ui/form";

const statusStyles: Record<OfferLetterStatus, string> = {
  Active: "bg-emerald-50 text-emerald-600",
  Paused: "bg-orange-50 text-orange-600",
};

export default function ViewOfferLettersTable({
  onEdit,
  onAddNew,
}: {
  ...
  const [rows, setRows] = useState(offerLetters);

  const filtered = rows.filter(
    (r) => typeFilter === "All Types" || r.type === typeFilter
  );
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
        ...
        <div className="flex items-center gap-2">
          <div className="relative hidden sm:block">
            ...
            <input
              type="text"
              placeholder="Search Offer Letter ID..."
          ...
          >
            <Plus size={15} /> <span className="hidden sm:inline">New Offer Letter</span>
          </button>
        </div>
      </div>
        ...
        <table className="w-full text-sm min-w-[720px]">
          <thead>
            <tr className="text-left text-xs text-brand-gray border-b border-gray-100">
              <th className="py-2 px-1 font-medium">Offer Letter ID</th>
              <th className="py-2 px-1 font-medium">Offer Type</th>
              <th className="py-2 px-1 font-medium">Approval Mail</th>
              <th className="py-2 px-1 font-medium">Created Date</th>
          ...
          <tbody>
            {filtered.map((row) => (
              ...
              >
                ...
                <td className="py-2.5 px-1 text-brand-dark">{row.type}</td>
                <td className="py-2.5 px-1">
                  {row.approvalMail ? (
                    <button
            ...
            {filtered.length === 0 && (
              <tr>
                <td colSpan={6} className="py-8 text-center text-sm text-brand-gray">
                  No offer letters match this filter.
                </td>
              </tr>
            )}
          </tbody>
