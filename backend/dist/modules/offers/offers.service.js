...
let OffersService = class OffersService {
    ...
    approveOfferLevel(id, level, comments) {
        const offer = this.findOne(id);
        if (level === 'rtm')
            offer.approvalFlow.rtmStatus = 'A';
        if (level === 'l1')
            offer.approvalFlow.l1Status = 'A';
        if (level === 'l2')
            offer.approvalFlow.l2Status = 'A';
        if (level === 'l3')
            offer.approvalFlow.l3Status = 'A';
        if (level === 'cm') {
